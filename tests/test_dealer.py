...
def test_create_game():
    ...
    case.assertDictEqual(dealer.create_game(3), {"id": 3, "winner": None, "rounds": 0})
...
def test_get_player_guess(popen):
    ...
    case.assertSetEqual(guessed_suspects, {"hbu", "lel", "pto"})
...
def test_add_card_to_data():
    ...
    case.assertListEqual(
        data["cards"], [{"suspects": tuple(suspects), "match_count": match_count}]
    )
...
class TestRunGame(object):
    ...
    def test_run_game(
        self, get_player_guess, build_data_object, create_deck, create_game
    ):
...
def test_get_sorted_player_wins():
    ...
    case.assertListEqual(all_wins, [("P2", 3), ("P3", 2), ("P1", 1)])
...
def test_create_players():
    ...
    for p, (program, player) in enumerate(zip(programs, players)):
        case.assertDictEqual(
            player, {"program": program, "wins": 0, "id": "P{}".format(p + 1)}
        )
